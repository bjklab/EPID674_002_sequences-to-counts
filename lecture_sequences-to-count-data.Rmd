...
- in contrast, whole-genome “shotgun” metagenomic sequencing involves sequencing all DNA present (i.e., “random” shearing and amplification to produce DNA library)
...
---
Ashelford Appl Env Micro 2005.


---
Yarza P Nat Rev Micro 2014.
...
Coined by Smith & Sokol, Principles of Numerical Taxonomy (1963): avoid definition that refers to established taxa.
...
SortMeRNA (2012): open-source replacement for Uclust.
CROP (2011) - Gaussian mixture models.


---
Methods of Forming OTUs (continued)
M-pick (2013): variable cluster size.
Swarm (2014): uses single-linkage clustering, later splits clusters based on mixture models
… newer methods to define “sequence variants” … to be discussed at the end of this class.


---
De Novo versus Reference-Based
If very large number of sequences, clustering into “de novo” OTUs may be prohibitively slow.
Why not just compare sequences to a reference database?


---
De Novo versus Reference-Based
He J. Microbiome 2015: “many hierarchical and greedy clustering methods produce unstable OTUs, with membership that depends on the number of sequences clustered…. the closed-reference method is the only one that produces completely stable OTUs, with the caveat that sequences that do not match a pre-existing reference sequence collection are discarded.”


---
Han M Sci Reports 2016.
...
Validity?
De novo OTUs?
De novo OTUs from rarefied reads?
Reference-based OTUs?
Which clustering algorithm?
Which reference database?
OTU clusters are defined across the entire study… what happens when you re-analyze with new companion specimens?


---
Let’s Make an OTU Table
...
so many specimens that a single barcode won’t suffice -- you have to barcode both forward and reverse reads(barcodes are “error-correcting” Golay codes)
...
Let’s Make an OTU Table
File:
“epid_674_lecture2-making_otus.Rmd”
Goals:
...
“a common early step in amplicon-based microbial community analysis... to reduce the run time of subsequent analysis steps”
many methods: OTUCLUST, Swarm, SUMACLUST, SortMeRNA, UCLUST, USEARCH, UPARSE
OTUs are “emergent features of the dataset”
...
OTUs are “properties of a reference database”
...
nevertheless, “consistent labels because SVs represent a biological reality that exists outside of the data being analyzed”
...
“Supervised” oligotyping (prior method):
can only be applied to closely related taxa (“many high-entropy locations among distant organisms makes supervision step arduous”)
requires a prelim OTU clustering or taxonomic classification to identify related taxa; “oligotyping is not a stand-alone approach”
“Unsupervised” oligotyping (new method):
Minimum Entropy Decomposition (MED) efficiently partitions marker gene datasets into ‘MED nodes’ (homogeneous OTUs)
...
“Clustering reads into OTUs underexploits quality of modern seq data”:
...
from this, identify “candidate sequences”: those whose presence cannot be explained by a substitution-only error model
...
---
Tikhonov et al ISME Journal 2015
Substitution error model: “error clouds” around high-abundance sequences
...
exclude positions ~ strongest outliers; use remaining positions to estimate the error rates… probability of each type of the error
...
rather than quality filtering and construction of OTUs, implement a “quality-aware model of Illumina amplicon errors”
...
computational tractability: don’t need to re-assign every time you split or combine data sets
meta-analysis and replication: association between “denovo123” and condition of interest cannot be validated independently
forward prediction: predictive biomarkers only useful if they can be applied to new data; as above, OTUs can’t move to new data set(though CR-OTUs can be extended to new data, any OTUs absent from reference database cannot)
...
- __Reflection:__ Why might one prefer assignment of metagenomic reads over contig assembly? Why might one prefer contig assembly?
